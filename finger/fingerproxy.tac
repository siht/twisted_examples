...
class FingerClient(protocol.Protocol):
    ...
    def connectionLost(self, reason):
        data = "".join([i.decode('utf-8') for i in self.buf])
        self.factory.gotData(data.encode())
...
def main():
    # corre el finger.tac primero
    # sudo /your_venv/twisted/bin/twistd -ny fingerproxy.tac --pidfile=algunnombreraro.pid
    global application
